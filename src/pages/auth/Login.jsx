  ...
  const loginHandle = async (e) => {
    ...
    if (email !== '' && password !== '') {
      await axios.post('https://pmb-api.politekniklp3i-tasikmalaya.ac.id/auth/login/v2', {
        ...
        .then((response) => {
          localStorage.setItem('LP3IPSY:token', response.data.token)
      ...
      loading ? (
        ...
        <main className='flex flex-col items-center justify-center bg-gradient-to-b from-lp3i-400 via-lp3i-200 to-lp3i-400 h-screen p-5 space-y-5'>
          ...
          <section className='max-w-lg mx-auto text-center space-y-1'>
            <h2 className='font-bold text-3xl text-white drop-shadow'>Psikotest Online: Kecerdasan</h2>
            <p className='text-white text-sm drop-shadow'>Platform tes online LP3I ini dirancang khusus untuk mengetahui jenis kecerdasan yang siswa/i miliki.</p>
          </section>
